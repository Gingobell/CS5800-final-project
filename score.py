...
def score(mp3):
    ...
    # ---- ENERGY ----
    # Compute RMS (Root Mean Square) amplitude = average loudness
    rms = librosa.feature.rms(y=y, frame_length=2048, hop_length=HOP).mean()

    # Normalize energy to [0,1] using log compression
    # This reduces the impact of extreme loudness differences
    energy = float(np.clip(
        (np.log1p(rms) - np.log(1 + 1e-4)) / (np.log(1 + 0.1) - np.log(1 + 1e-4)),
        0, 1
    ))
